...
async def lifespan(app: FastAPI):
    ...
    log_to_file("Initializing trading bot...")
    ...
    log_to_file("Bot successfully initialized and ready to receive trading signals.")
...
async def webhook(request: Request):
    ...
    try:
        ...
        checker = await asyncio.to_thread(process_signal) # why did i change this to thread
        
        print(f"checking event {event}")
        log_to_file(f"got a {event} alert; with data: {data}")
        ...
        # Case of market close order/ TODO: case of shorting
            ...
            close_order = await bot.leveraged_market_close_Order(ticker, "buy", abs(size))
            ...
            bot.cancelLimitOrders(deviations)
